...
var MarkdownParts = [
  ...
  #### Service Fabric Managed - SCE Common Infra.
...
//     LogsDashboardParts: filter(LogsDashboardParts, part => part.ha != 1 )
//     MonitorChartPart: filter(MonitorChartParts, part => part.ha != 1 )
//     IFrameParts: filter(IFrameParts, part => part.ha != 1 )
//     MarkdownParts: filter(MarkdownParts, part => part.ha != 1 )
//   }
// }
...
output isHA bool = isHA
// output dash array = filter(LogsDashboardParts, part => (! contains(part,'ha')) || (isHA && part.ha == 1) || (( ! isHA ) && part.ha == 0) )
// output mon array = filter(MonitorChartParts, part => (! contains(part,'ha')) || (isHA && part.ha == 1) || (( ! isHA ) && part.ha == 0) )
// output iframe array = filter(IFrameParts, part => (! contains(part,'ha')) || (isHA && part.ha == 1) || (( ! isHA ) && part.ha == 0) )
output markdown array = filter(MarkdownParts, part => (contains(part, 'ha') && bool(part.ha) == bool(isHA))) // ( (! contains(part,'ha')) || bool(part.ha) == isHA) )
